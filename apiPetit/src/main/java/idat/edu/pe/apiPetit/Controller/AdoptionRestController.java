...
public class AdoptionRestController {
    ...
    public ResponseEntity<AdoptionDTO> saveAdoption(@PathVariable(name = "userId") Integer userId, @PathVariable(name = "stateId") Integer stateId, @PathVariable(name = "petId") Integer petId, @RequestBody AdoptionDTO adoptionDTO) {
        return new ResponseEntity<>(adoptionService.createAdoption(petId, stateId, userId, adoptionDTO), HttpStatus.CREATED);
    }
    ...
    @RequestMapping(path = "/listaUbicaciones/{idState}", method = RequestMethod.GET)
    public ResponseEntity<?> obtenerUbicaciones(@PathVariable Integer idState) {
        return new ResponseEntity<>(adoptionService.obtenerUbicaciones(idState), HttpStatus.OK);
    }
}
